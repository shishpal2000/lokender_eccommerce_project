...
const Cart = () => {
  ...
  // Increase
  const inc = async (id) => {
    let url = `${Baseurl()}api/v1/cart/increase/${id}`;
  ...
  const createOrderHandler = async() => {
    ...
    try {
      ...
      // Set the 'Bearer' type in the Authorization header
  ...
  const res = await axios.post(url, body, {
    headers: {
      ...
      // const res = await axios.get(url, {
      //   headers: {
      //     Authorization: `BearerToken ${localStorage.getItem("boon")}`,
      //   },
      // });
      // setCartPro(res.data.data.products);
  ...
  // Decrease
  const dec = async (id) => {
    let url = `${Baseurl()}api/v1/cart/decrease/${id}`;
    let data = { quantity: qua };
    try {
      const res = await axios.patch(url, data, {
        headers: {
      ...
      <div className="cartcont">
        <div className="cartcontl">
          ...
          {cartProducts.length  && cartProducts.map((product, i) => {
            return <div className="cartcont2">
            <div className="cartcont2l">
              <div className="cartprod">
                ...
                <div className="cartprodr">
                  ...
                  <p>Product color detail: {product.productId.color.join(",")}</p>
          ...
          {console.log("cart details return", cartDetails)}
        ...
        {cartDetails.length > 0 && cartDetails.map((product, i) => {
          ...
          <button onClick={createOrderHandler} className="bg-gray-700 text-white text-sm py-[2px] px-2 rounded-md" style={{marginTop: "20px"}}>Place Order</button>
        </div>
        })}
